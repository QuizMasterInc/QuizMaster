...
export default function CustomQuiz () {
  ...
  const createQuizDataObject = (quizData) => {
    ...
    for (let i = 0; i < quizData.length; i++) {
      ...
      const type = questionDetailsArray[6];
      ...
      if (type === 'DragAndDrop') {
        // Store all non-empty drag options (could be answers, targets, etc.)
        questionObject.drag_options = questionDetailsArray.slice(1, 5).filter(opt => opt.trim() !== '');
      }
  ...
  useEffect(() => {
    console.log(quizData); // This just shows the updated state of quizData
  }, [quizData]);
